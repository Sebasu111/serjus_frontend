...
const InduccionDocumentosContainer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                await axios.put(
                    `${API}/inducciondocumentos/${editingId}/`,
                    payload
                );
    ...
    const toggleEstado = async (row, nuevo) => {
        try {
            await axios.put(
                `${API}/inducciondocumentos/${row.idinducciondocumento}/`,
                { ...row, estado: nuevo, idusuario: row.idusuario ?? 1 }
            );
    ...
    return (
        <Layout>
            <SEO title="Hope – Inducción Documentos" />
            <div
                className="wrapper"
                style={{
                    display: "flex",
