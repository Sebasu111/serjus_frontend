...
const TerminacionLaboralContainer = () => {
    ...
    const fetchTerminaciones = async () => {
        try {
            const res = await axios.get(
                "http://127.0.0.1:8000/api/terminacionlaboral/"
            );
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("¿Estás seguro de desactivar esta terminación?"))
            return;
        try {
            const terminacion = terminaciones.find(
                (t) => t.idterminacionlaboral === id
            );
            if (!terminacion) return;

            await axios.put(
                `http://127.0.0.1:8000/api/terminacionlaboral/${id}/`,
                {
                    ...terminacion,
                    estado: false,
                }
            ...
            console.error(
                "Error al desactivar terminación:",
