...
const CriterioEvaluacionContainer = () => {
    ...
    const fetchCriterios = async () => {
        try {
            const res = await axios.get(
                "http://127.0.0.1:8000/api/criterioevaluacion/ "
            );
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("¿Estás seguro de desactivar este criterio?"))
            return;
        try {
            ...
            await axios.put(
                `http://127.0.0.1:8000/api/criterioevaluacion/ ${id}/`,
                {
                    ...crit,
                    estado: false,
                }
