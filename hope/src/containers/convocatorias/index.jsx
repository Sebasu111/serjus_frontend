...
const ConvocatoriasContainer = () => {
  ...
  const fetchList = async () => {
    try{
      ...
      setRows([]); setMensaje("Error al cargar las convocatorias");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      setForm({ nombreconvocatoria:"", fechainicio:"", fechafin:"", idpuesto:"", estado:true, idusuario:1 });
  ...
  const toggleEstado = async (row, nuevo) => {
    try{
      await axios.put(`${API}/convocatorias/${row.idconvocatoria}/`, { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 });
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:900, margin:"0 auto" }}>
            ...
            <div style={{ background:"#fff", padding:40, borderRadius:12, boxShadow:"0 4px 20px rgba(0,0,0,0.1)", marginBottom:40 }}>
              <h2 style={{ textAlign:"center", marginBottom:30 }}>{editingId ? "Editar convocatoria" : "Registrar convocatoria"}</h2>
            ...
            <div style={tableWrapper}>
              <h3 style={{ marginBottom:20, textAlign:"center" }}>Convocatorias</h3>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>{["Nombre","Inicio","Fin","Puesto","Estado","Acciones"].map((h)=><th key={h} style={thStyle}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {Array.isArray(rows)&&rows.length? rows.map((r)=>(
                    <tr key={r.idconvocatoria}>
                      <td style={tdStyle}>{r.nombreconvocatoria}</td>
                    ...
                    <tr><td colSpan="6" style={{ textAlign:"center", padding:20 }}>Sin registros</td></tr>
                  )}
                </tbody>
              </table>
            </div>
