...
const AspirantesContainer = () => {
  ...
  const fetchList = async () => {
    try {
      ...
      console.error("Error aspirantes:", e);
      setRows([]); setMensaje("Error al cargar los aspirantes");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setMensaje("Aspirante actualizado");
      } else {
        await axios.post(`${API}/aspirantes/`, payload);
        setMensaje("Aspirante registrado");
      }
      setForm({ nombreaspirante:"", apellidoaspirante:"", nit:"", telefono:"", email:"", estado:true, idusuario:1 });
      setEditingId(null);
      fetchList(); window.scrollTo({ top:0, behavior:"smooth" });
    } catch (e) {
      console.error("Error guardar aspirante:", e.response?.data || e);
  ...
  const handleEdit = (row) => {
    setForm({
      nombreaspirante: row.nombreaspirante ?? "",
      apellidoaspirante: row.apellidoaspirante ?? "",
      nit: row.nit ?? "",
    ...
    setEditingId(row.idaspirante);
    window.scrollTo({ top:0, behavior:"smooth" });
  };

  const toggleEstado = async (row, nuevo) => {
    try{
      await axios.put(`${API}/aspirantes/${row.idaspirante}/`, { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 });
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:900, margin:"0 auto" }}>
            ...
            <div style={{ background:"#fff", padding:40, borderRadius:12, boxShadow:"0 4px 20px rgba(0,0,0,0.1)", marginBottom:40 }}>
              <h2 style={{ textAlign:"center", marginBottom:30 }}>{editingId ? "Editar aspirante" : "Registrar aspirante"}</h2>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                  <div>
                    <label>Nombre</label>
                    <input name="nombreaspirante" value={form.nombreaspirante} onChange={onChange} required style={inputStyle}/>
                    <label>Apellido</label>
                    <input name="apellidoaspirante" value={form.apellidoaspirante} onChange={onChange} required style={inputStyle}/>
                    <label>NIT</label>
            ...
            <div style={tableWrapper}>
              <h3 style={{ marginBottom:20, textAlign:"center" }}>Aspirantes</h3>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>{["Nombre","Apellido","NIT","Teléfono","Email","Estado","Acciones"].map((h)=><th key={h} style={thStyle}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {Array.isArray(rows)&&rows.length? rows.map((r)=>(
                    <tr key={r.idaspirante}>
                      <td style={tdStyle}>{r.nombreaspirante}</td>
                      <td style={tdStyle}>{r.apellidoaspirante}</td>
                      <td style={tdStyle}>{r.nit}</td>
