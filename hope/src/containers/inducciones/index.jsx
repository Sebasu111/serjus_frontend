...
const InduccionesContainer = () => {
  ...
  const fetchList = async () => {
    try{
      const r = await axios.get(`${API}/inducciones/`);
      const data = Array.isArray(r.data) ? r.data :
                   Array.isArray(r.data?.results) ? r.data.results : [];
      setRows(data);
    }catch(e){
      console.error("Error inducciones:", e);
      setRows([]); setMensaje("Error al cargar las inducciones");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      const payload = { ...form };
      if (editingId) {
        await axios.put(`${API}/inducciones/${editingId}/`, payload);
        setMensaje("Inducción actualizada");
      } else {
        await axios.post(`${API}/inducciones/`, payload);
        setMensaje("Inducción registrada");
      }
      setForm({ fechainicio:"", fechafin:"", estado:true, idusuario:1 });
  ...
  const toggleEstado = async (row, nuevo) => {
    try{
      await axios.put(`${API}/inducciones/${row.idinduccion}/`, { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 });
      ...
      <div className="wrapper" style={{ display:"flex", flexDirection:"column", minHeight:"100vh" }}>
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:900, margin:"0 auto" }}>
            {/* FORM */}
            <div style={card}>
              <h2 style={h2}>{editingId ? "Editar inducción" : "Registrar inducción"}</h2>
              {mensaje && <p style={msg(mensaje)}>{mensaje}</p>}
              <form onSubmit={handleSubmit}>
                <div style={grid2}>
                  <div>
                    <label>Fecha inicio</label>
                    <input type="date" name="fechainicio" value={form.fechainicio} onChange={onChange} required style={input}/>
                  </div>
                  <div>
                    <label>Fecha fin</label>
                    <input type="date" name="fechafin" value={form.fechafin} onChange={onChange} style={input}/>
            ...
            {/* TABLA */}
            <div style={tableWrap}>
              <h3 style={h3}>Inducciones</h3>
              <table style={tbl}>
                <thead>
                  <tr>{["Inicio","Fin","Estado","Acciones"].map((h)=><th key={h} style={th}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {rows?.length ? rows.map((r)=>(
                    <tr key={r.idinduccion}>
                      <td style={td}>{r.fechainicio}</td>
                      <td style={td}>{r.fechafin || "-"}</td>
