...
const HistorialPuestoContainer = () => {
  const [form, setForm] = useState({
    fechainicio: "", fechafin: "", salario: "", observacion: "",
    estado: true, idusuario: 1, idempleado: "", idpuesto: ""
  });
  ...
  const fetchList = async () => {
    try{
      ...
      setRows([]); setMensaje("Error al cargar el historial de puestos");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      setForm({ fechainicio:"", fechafin:"", salario:"", observacion:"", estado:true, idusuario:1, idempleado:"", idpuesto:"" });
      setEditingId(null);
      fetchList();
  ...
  const handleEdit = (row) => {
    setForm({
      fechainicio: row.fechainicio ?? "", fechafin: row.fechafin ?? "",
      salario: row.salario ?? "", observacion: row.observacion ?? "",
      estado: !!row.estado, idusuario: row.idusuario ?? 1,
      idempleado: row.idempleado ?? "", idpuesto: row.idpuesto ?? ""
    });
  ...
  const toggleEstado = async (row, nuevo) => {
    try{
      await axios.put(`${API}/historialpuestos/${row.idhistorialpuesto}/`, { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 });
  ...
  return (
    <Layout>
      <SEO title="Hope – Historial de Puestos" />
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:900, margin:"0 auto" }}>
            ...
            <div style={{ background:"#fff", padding:40, borderRadius:12, boxShadow:"0 4px 20px rgba(0,0,0,0.1)", marginBottom:40 }}>
              <h2 style={{ textAlign:"center", marginBottom:30 }}>{editingId ? "Editar historial" : "Registrar historial"}</h2>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                  <div>
                    ...
                    <label>Salario</label>
                    <input name="salario" value={form.salario} onChange={onChange} required style={inputStyle}/>
                  </div>
                  <div>
                    <label>Observación</label>
                    <input name="observacion" value={form.observacion} onChange={onChange} required style={inputStyle}/>
                    <label>ID Empleado</label>
                    <input name="idempleado" value={form.idempleado} onChange={onChange} required style={inputStyle}/>
                    <label>ID Puesto</label>
                    <input name="idpuesto" value={form.idpuesto} onChange={onChange} required style={inputStyle}/>
            ...
            <div style={tableWrapper}>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>
                    {["Inicio","Fin","Salario","Obs.","Empleado","Puesto","Estado","Acciones"].map((h)=><th key={h} style={thStyle}>{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {Array.isArray(rows)&&rows.length? rows.map((r)=>(
                    ...
                    <tr><td colSpan="8" style={{ textAlign:"center", padding:20 }}>Sin registros</td></tr>
                  )}
                </tbody>
              </table>
            </div>
