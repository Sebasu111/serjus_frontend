...
const ContratosContainer = () => {
  const [form, setForm] = useState({
    fechainicio: "", fechafirma: "", fechafin: "",
    tipocontrato: "", estado: true, idusuario: 1, idhistorialpuesto: ""
  });
  ...
  const fetchList = async () => {
    try{
      ...
      setRows([]); setMensaje("Error al cargar los contratos");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      setForm({ fechainicio:"", fechafirma:"", fechafin:"", tipocontrato:"", estado:true, idusuario:1, idhistorialpuesto:"" });
      setEditingId(null);
  ...
  const toggleEstado = async (row, nuevo) => {
    try{
      await axios.put(`${API}/contratos/${row.idcontrato}/`, { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 });
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:900, margin:"0 auto" }}>
            ...
            <div style={{ background:"#fff", padding:40, borderRadius:12, boxShadow:"0 4px 20px rgba(0,0,0,0.1)", marginBottom:40 }}>
              <h2 style={{ textAlign:"center", marginBottom:30 }}>{editingId ? "Editar contrato" : "Registrar contrato"}</h2>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                  <div>
                    ...
                    <input type="date" name="fechafirma" value={form.fechafirma} onChange={onChange} required style={inputStyle}/>
                    <label>Fecha fin</label>
                    <input type="date" name="fechafin" value={form.fechafin} onChange={onChange} style={inputStyle}/>
                  </div>
                  <div>
                    <label>Tipo de contrato</label>
                    <input name="tipocontrato" value={form.tipocontrato} onChange={onChange} required style={inputStyle}/>
                    <label>ID Historial Puesto</label>
                    <input name="idhistorialpuesto" value={form.idhistorialpuesto} onChange={onChange} style={inputStyle}/>
            ...
            <div style={tableWrapper}>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>{["Inicio","Firma","Fin","Tipo","Historial","Estado","Acciones"].map((h)=><th key={h} style={thStyle}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {Array.isArray(rows)&&rows.length? rows.map((r)=>(
                    <tr key={r.idcontrato}>
                      <td style={tdStyle}>{r.fechainicio}</td>
                      <td style={tdStyle}>{r.fechafirma}</td>
                      <td style={tdStyle}>{r.fechafin || "-"}</td>
                      <td style={tdStyle}>{r.tipocontrato}</td>
                      <td style={tdStyle}>{r.idhistorialpuesto || "-"}</td>
                    ...
                    <tr><td colSpan="7" style={{ textAlign:"center", padding:20 }}>Sin contratos</td></tr>
                  )}
                </tbody>
              </table>
            </div>
