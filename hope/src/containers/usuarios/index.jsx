...
const UsuariosContainer = () => {
  const [form, setForm] = useState({
    nombreusuario: "",
    email: "",
    contrasena: "",       // si tu API no expone este campo, bórralo
    idrol: "",
    idempleado: "",
    estado: true,
    idusuario: 1,        // usuario que realiza la acción (auditoría)
  });
  ...
  const fetchList = async () => {
    try{
      ...
      setRows([]); setMensaje("Error al cargar usuarios");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      const payload = { ...form };
      if (!payload.contrasena) delete payload.contrasena; // evita enviar vacío al editar
      if (editingId) {
      ...
      setForm({ nombreusuario:"", email:"", contrasena:"", idrol:"", idempleado:"", estado:true, idusuario:1 });
  ...
  const handleEdit = (row) => {
    setForm({
      ...
      email: row.email ?? "",
      contrasena: "", // no mostramos hash; solo permitir cambiar si lo escribe
      idrol: row.idrol ?? "",
      idempleado: row.idempleado ?? "",
      estado: !!row.estado,
      idusuario: row.idusuario ?? 1,
    });
    setEditingId(row.idusuario);
    window.scrollTo({ top:0, behavior:"smooth" });
  };

  const toggleEstado = async (row, nuevo) => {
    try{
      const payload = { ...row, estado:nuevo, idusuario: row.idusuario ?? 1 };
      delete payload.contrasena;
      await axios.put(`${API}/usuarios/${row.idusuario}/`, payload);
      ...
      <div className="wrapper" style={{ display:"flex", flexDirection:"column", minHeight:"100vh" }}>
        ...
        <main style={{ flex:1, padding:"60px 20px", background:"#f0f2f5" }}>
          <div style={{ maxWidth:1000, margin:"0 auto" }}>
            <div style={card}>
              <h2 style={h2}>{editingId ? "Editar usuario" : "Registrar usuario"}</h2>
              {mensaje && <p style={msg(mensaje)}>{mensaje}</p>}
              <form onSubmit={handleSubmit}>
                <div style={grid2}>
                  <div>
                    <label>Nombre de usuario</label>
                    <input name="nombreusuario" value={form.nombreusuario} onChange={onChange} required style={input}/>
                    <label>Email</label>
                    <input name="email" value={form.email} onChange={onChange} required style={input}/>
                    <label>Contraseña {editingId ? "(dejar vacío si no cambia)" : ""}</label>
                    <input type="password" name="contrasena" value={form.contrasena} onChange={onChange} style={input}/>
                  </div>
                  <div>
                    <label>ID Rol</label>
                    <input name="idrol" value={form.idrol} onChange={onChange} required style={input}/>
                    <label>ID Empleado</label>
                    <input name="idempleado" value={form.idempleado} onChange={onChange} style={input}/>
            ...
            <div style={tableWrap}>
              <h3 style={h3}>Usuarios</h3>
              <table style={tbl}>
                <thead>
                  <tr>{["Usuario","Email","Rol","Empleado","Estado","Acciones"].map((h)=><th key={h} style={th}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {rows?.length ? rows.map((r)=>(
                    <tr key={r.idusuario}>
                      <td style={td}>{r.nombreusuario}</td>
                      <td style={td}>{r.email}</td>
                      <td style={td}>{r.idrol}</td>
                      <td style={td}>{r.idempleado || "-"}</td>
                      <td style={{ ...td, textAlign:"center", color:r.estado?"green":"red", fontWeight:600 }}>{r.estado ? "Activo":"Inactivo"}</td>
                    ...
                    <tr><td colSpan="6" style={{ textAlign:"center", padding:20 }}>Sin usuarios</td></tr>
                  )}
                </tbody>
              </table>
            </div>
